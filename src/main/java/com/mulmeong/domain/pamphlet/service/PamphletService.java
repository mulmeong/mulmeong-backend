...
public class PamphletService {
    ...
    public PamphletDetail detail(Long userId, Long pamphletId) {
        ...
        User author = users.getById(userId)
                .orElseThrow(() -> new BusinessException(ErrorCode.PAMPHLET_NOT_FOUND));
        return detail(pamphlet, selected, author, userId, true);
    }
    ...
    public PamphletDetail shared(String token, Long viewerId) {
        ...
        return detail(p, selected, author, viewerId, mine);
    }
    ...
    private PamphletDetail detail(Pamphlet p, List<Place> selected, User author, Long viewerId, boolean mine) {
        int onsens = (int) selected.stream().filter(x -> x.getPlaceType() == PlaceType.ONSEN).count();
        List<Long> placeIds = selected.stream().map(Place::getId).toList();
        Set<Long> favoritePlaceIds = favoriteService.getFavoritePlaceIds(viewerId, placeIds);
        Map<Long, ReviewAggregate> reviewsByPlaceId = selected.isEmpty()
                ? Map.of()
                : reviewAggregates.findByPlaceIds(placeIds)
                        .stream()
        ...
        return new PamphletDetail(mine ? p.getId() : null, p.getShareToken(), p.getTitle(), toInteger(p.getPartySize()), p.getTravelDate(),
                ...
                java.util.stream.IntStream.range(0, selected.size())
                        .mapToObj(i -> placeItem(selected.get(i), i + 1,
                                reviewsByPlaceId.get(selected.get(i).getId()),
                                favoritePlaceIds.contains(selected.get(i).getId())))
                        .toList(),
    ...
    private PamphletPlaceItem placeItem(Place p, int seq, ReviewAggregate review, boolean favorite) {
        ...
        return new PamphletPlaceItem(seq, p.getId(), p.getExternalId(), p.getContentTypeId(), type,
                ...
                p.isRegisteredOnsen(), p.getSido(), p.getSigungu(), p.getPhone(), p.getHomepageUrl(), p.getHours(),
                p.getHoliday(), p.getParkingInfo(),
                ...
                access, placeImages, thumbnail, p.getRegionComment(), p.getNotes(),
                trustedAnnualVisitors(p.getAnnualVisitors()), favorite, reviewSummary);
    }

    private Integer trustedAnnualVisitors(Integer annualVisitors) {
        return annualVisitors == null || annualVisitors <= 0 ? null : annualVisitors;
    }
